...
| Blocklist regexes could be stateful | **Checked Aug 2026: none are.** `check()` calls `regex.exec()`, which on a `/g` or `/y` pattern carries `lastIndex` between calls and would let ads through on alternating checks. All 42 patterns across `allowUrl`, `allowText`, `blockHostname`, `blockUrl` and `blockText` are flagless. Re-check if a pattern is ever added with a flag. |
| `parseVotes` handles the shapes it claims | **Verified Aug 2026** against all 13 documented cases - `+406`, thousands separators, `k`/`M`, whitespace. Non-numeric input yields `NaN`, which is correct: every downstream comparison is then false, so no highlight applies. |
...
| Three menu colors depend on Slickdeals' own CSS variables, with no fallback | **Latent, and this class has already bitten once.** `.changes > *` uses `var(--mainNavTextColor)`, and `.sdp-menu .slickdealsHeaderDropdownItem` uses `var(--hamburgerTextColor)` and `var(--dropdownTextColor)`. None of the three is defined by this script - they come from the site. If Slickdeals renames or drops one, the declaration becomes invalid at computed-value time and the color silently falls back to inherited, with no error anywhere. "Comment entries washed out on the light panel", fixed in 26.11.33, was exactly this failure shape. The fix is a fallback - `var(--mainNavTextColor, #333)` - but picking the colors is a CSS change, and **§5 forbids shipping CSS on reasoning alone**, so it waits for someone with a browser open. Checked Aug 2026: the other eleven custom properties the CSS uses are all defined by this script. |
