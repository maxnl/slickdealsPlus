...
| A post-content link's cached destination is never re-validated | By design since 26.11.15: it was resolved under an id of its own, so it cannot be another link's answer and is read back unchecked. Worth keeping in proportion - this is not a safety net that was lost. No cached-value check existed at all before 26.11.13, that version's check was the broken one that rejected 228 of 287 links, and the working version of it was released at 08:28 and guarded at 13:22 the same day: **four hours fifty-four minutes** of released life. The practical position is unchanged from the fork's entire history, with the 26.11.16 purge clearing out anything written before the guard. What it does mean is that a wrong answer to a *unique* lookup - much less likely than a collision, but not impossible - would persist until the 5000-entry cap evicted it. Symptom: one forum-post link resolving somewhere wrong and staying wrong across reloads while its neighbours are fine. Remedy: `localStorage.removeItem("slickdeals+links")`. A TTL on cache entries would close it properly if it ever bites. |
