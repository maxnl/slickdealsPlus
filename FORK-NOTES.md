...
| A post-content link's cached destination is never re-validated | By design since 26.11.15: it was resolved under an id of its own, so it cannot be another link's answer and is read back unchecked. The consequence is that there is no self-correcting path left for those entries. If the resolver ever returns a wrong destination for a *unique* id - much less likely than a collision, but not impossible - it stays cached until the 5000-entry cap evicts it or the cache is cleared by hand. The 26.11.16 purge cleans out everything written before the guard existed, but it fires once, on upgrade. Symptom to watch for: a single forum-post link that resolves somewhere wrong and stays wrong across reloads while everything around it is fine. Remedy is `localStorage.removeItem("slickdeals+links")`. A TTL on cache entries would close it properly if it ever bites. |
